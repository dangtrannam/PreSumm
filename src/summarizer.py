...
if __name__ == '__main__':
    ...
    all_files = glob.glob(os.path.join('D:\\Dev\\MSE_projects\\New folder\\PreSumm\\bert_data\\cnndm', '*'))
    print('Files In Input Dir: ' + str(len(all_files)))
    for file in all_files:
        with open(file) as f:
            source=f.read().rstrip()

        data_builder.str_format_to_bert(  source, args, 'D:\\Dev\\MSE_projects\\New folder\\PreSumm\\bert_data_test\\cnndm.test.0.bert.pt') 
        args.bert_data_path= 'D:\\Dev\\MSE_projects\\New folder\\PreSumm\\bert_data_test\\cnndm'
